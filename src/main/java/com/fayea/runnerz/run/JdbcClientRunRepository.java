...
public class JdbcClientRunRepository {
    ...
    private final Logger logger = Logger.getLogger(JdbcClientRunRepository.class.getName());
    ...
    public Optional<Run> findById(Integer id) {
        return jdbcClient.sql("select * from Run where id = :id").param("id", id)
    ...
    public void update(Run run, Integer id) {
        var updated= jdbcClient.sql("update Run set title=?, started_on=?, completed_on=?, miles=?,location=? where id =:id")
                .params(List.of(
                        run.title(),
                        run.startedOn(),
                        run.completedOn(),
